...
namespace BrainViewer
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        readonly int X = 29, Y = 30, Z = 31;//X = 256, Y = 256, Z = 38;
        float scale = 2;//50; // model scale
        ...
        protected override void LoadContent()
        {
            ...
            string filePath = "helix-ten.raw";//"Patient_A\\A-ten-mask.raw";    
            //File Dimensions
            int length = X * Y * Z;
            ...
            #region Create World Entities
            {
                ...
                StreamWriter transWriter = new StreamWriter("transforms.txt");
                transWriter.WriteLine("#transforms   m11 m12 m13 m21 m22 m23 m31 m32 m33");
                    ...
                    for (int y = 0; y < Y; y++)
                    {
                        for (int x = 0; x < X; x++)
                        {
                            ...
                            if (confidence[listIndex] == 1.0)
                            {
                                ...
                                float l1 = (float)eigenValues[listIndex][0].Real; float l2 = (float)eigenValues[listIndex][1].Real; float l3 = (float)eigenValues[listIndex][2].Real;
                                stretch = XNAMatrix.CreateScale(new Vector3(l1 * scale, l2 * scale, l3 * scale));
                                XNAMatrix transforms = stretch * rotate * position;

                                transWriter.Write(transforms.M11.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M12.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M13.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M21.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M22.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M23.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M31.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M32.ToString()); transWriter.Write(" ");
                                transWriter.Write(transforms.M33.ToString()); 
                                transWriter.WriteLine("");

                                entities[index] = new WorldEntity(model, transforms, XNAMatrix.Identity);
                                index++;
                            }
                        }
                    }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                Vector3 toVector = camera.cameraTarget - camera.cameraPosition;
                XNAMatrix rotate = XNAMatrix.CreateFromAxisAngle(toVector, 3.141592653589f / 5000);
                XNAQuaternion q = XNAQuaternion.CreateFromRotationMatrix(rotate);
            ...
            {
                Vector3 toVector = camera.cameraTarget - camera.cameraPosition;
                XNAMatrix rotate = XNAMatrix.CreateFromAxisAngle(toVector * -1 , 3.141592653589f / 5000);
                XNAQuaternion q = XNAQuaternion.CreateFromRotationMatrix(rotate);
                Vector3 newUp = Vector3.Transform(camera.cameraUpVector, q);
                camera.cameraUpVector = newUp;
            }
